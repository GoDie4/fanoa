...
export const ListaClientes = (): JSX.Element => {
  ...
  const getAllProductos = async (): Promise<void> => {
    ...
    const request = await axios.post(`${Global.url}/getClientes`, data, {
      headers: {
  ...
  function handleImageError(
    ...
    // Cambiar la imagen de la etiqueta img por la imagen de reemplazo
    event.currentTarget.src = `${logo_white}`;
  }
    ...
    <>
      ...
      {loadingComponents ? (
        ...
        <div className="p-8 bg-secondary-100 rounded-xl">
          <div className="hidden grid-cols-1 gap-4 p-4 mb-10 md:grid md:grid-cols-3">
          ...
          {filterDate().map((pro: categoriasValues) => (
            <div
              className="grid items-center grid-cols-1 gap-4 p-4 mb-4 md:grid-cols-3 bg-secondary-900 rounded-xl"
              key={pro.id}
            >
