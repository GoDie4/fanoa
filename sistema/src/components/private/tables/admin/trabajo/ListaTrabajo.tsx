// src/components/private/tables/admin/trabajo/ListaTrabajo.tsx
...
import { getAllTrabajoByCategoryId } from "./actions/getAllTrabajoByCategoryId";
import { deleteTrabajo } from "./actions/deleteTrabajo";
import { TrabajoResponse } from "./interface/trabajo.interface";
import useAuth from "../../../../../hooks/useAuth";

export const ListaTrabajo = (): JSX.Element => {
  ...
  const fetchTrabajos = async (categoriaId: string) => {
    setLoadingComponents(true);
    try {
      const data = await getAllTrabajoByCategoryId(categoriaId || "");
      setTrabajos(data);
  ...
  useEffect(() => {
    fetchTrabajos(categoriaSeleccionada);
    setTitle("Pasos de trabajo");
      ...
      {loadingComponents ? (
        ...
        <div className="p-8 bg-secondary-100 rounded-xl">
            ...
            {trabajos.map((trabajo) => (
              ...
              >
                <div className="relative aspect-video">
                  <img
                    src={trabajo.imagen ?? "/placeholder-image.jpg"}
                    alt={trabajo.titulo}
                    ...
                    {categoriaSeleccionada && (
                      ...
                      >
                        <MenuItem className="p-0 hover:bg-transparent">
                          <Link
                            to={`${categoriaSeleccionada}/editar/${trabajo.id}`}
                ...
                <div className="p-4">
                  ...
                  <p className="text-gray-400 text-sm truncate">{trabajo.descripcion}</p>
                  <p className="mt-2 text-xs text-gray-500">ID: {trabajo.id.split("-")[0]}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col justify-between gap-5 mt-8 md:flex-row md:gap-0 content_buttons">
            <p className="ml-1 text-md">{totalRegistros} Trabajos</p>
          </div>
        </div>
      )}
    </>
